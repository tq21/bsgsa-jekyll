...
Based on [Steve's No-Good-Very-Bad Jekyll Theme](https://github.com/svmiller/steve-ngvb-jekyll-template). UC Berkeley color palette is implemented in `css/main.scss`.

To deploy, build and upload:
- Build with `jekyll build` (requires [jekyll](http://jekyllrb.com/docs/installation/))
- Upload contents of `_site` to `public_html` via `ssh.ocf.berkeley.edu`.
